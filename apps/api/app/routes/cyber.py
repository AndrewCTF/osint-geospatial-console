...
async def _fetch_ioda(days: int) -> dict[str, Any]:
    ...
    # the caller can short-TTL-cache it and the caller degrades gracefully.
    try:
...
async def load_ioda(days: int = 7) -> dict[str, Any]:
    """Cached IODA outage fetch — the callable surface for in-process
    consumers (the instability scorer). Never call the route handler
    in-process; call this."""
    key = f"ioda:outages:{days}"
    result = await cache.get_or_fetch(key, 1800.0, lambda: _fetch_ioda(days))
